...
RUN apk update && apk add curl binutils build-base pkgconfig openssl-dev openssl-libs-static
...
RUN curl -L https://github.com/kubernetes-sigs/cri-tools/releases/download/${CRICTL_VERSION}/crictl-${CRICTL_VERSION}-linux-${ARCH}.tar.gz --output crictl-linux-${ARCH}.tar.gz
RUN tar zxvf crictl-linux-${ARCH}.tar.gz
